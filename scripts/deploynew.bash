...
(
		...
		stepresult=$(az storage account create --name $deployprefix$storageAccountNameSuffix --resource-group $resourceGroupName --location  $resourceGroupLocation --sku Standard_LRS --encryption-services blob --allow-blob-public-access false)
		...
		if [ -n "$useproxy" ]; then
			stepresult=$(az functionapp config appsettings set --name $faname --resource-group $resourceGroupName --settings FBI-DISABLE-BLOBTRIGGER=true FBI-STORAGEACCT=$(kvuri FBI-STORAGEACCT) FS-URL=$fsurl FS-TENANT-NAME=$(kvuri FP-SC-TENANT-NAME) FS-CLIENT-ID=$(kvuri FP-SC-CLIENT-ID) FS-SECRET=$(kvuri FP-SC-SECRET) FS-RESOURCE=$(kvuri FP-SC-RESOURCE))
		else
			stepresult=$(az functionapp config appsettings set --name $faname --resource-group $resourceGroupName --settings FBI-DISABLE-BLOBTRIGGER=true FBI-STORAGEACCT=$(kvuri FBI-STORAGEACCT) FS-URL=$fsurl FS-TENANT-NAME=$(kvuri FS-TENANT-NAME) FS-CLIENT-ID=$(kvuri FS-CLIENT-ID) FS-SECRET=$(kvuri FS-SECRET) FS-RESOURCE=$(kvuri FS-RESOURCE))
		fi
